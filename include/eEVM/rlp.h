    ...
    namespace rlp
    {
        ...
        namespace encode_details
        {
            ...
            ByteString encode_single(const T& t)
            {
                ...
                bytes.insert(bytes.begin(), length_as_bytes.begin(), length_as_bytes.end());
                return bytes;
            }

            // RLP-encode a list of arguments. Encode each separately, and return a
            // tuple of the results.
            template <typename... Ts, typename = std::enable_if_t<!is_tuple<std::decay_t<Ts>...>::value>>
            ...
            {
                return std::apply([](auto&&... entry) { return std::make_tuple(encode(entry)...); }, tup);
            }
        }
        ...
        ByteString encode(Ts&&... ts)
        {
            ...
            const auto nested_terms = encode_details::encode_multiple(std::forward<Ts>(ts)...);

            // Get total length by summing size of each term in tuple
            const auto total_length = std::apply([](auto&&... term) { return (term.size() + ... + 0); }, nested_terms);

            ByteString flattened;

            // Get concatenation by inserting each term's encoding
            std::apply(
              [&flattened](auto&&... term) { ((flattened.insert(flattened.end(), term.begin(), term.end()), ...)); },
              nested_terms);
        ...
        namespace decode_details
        {
            ...
            inline std::pair<Arity, size_t> decode_length(const uint8_t*& data, size_t& size);
            ...
            struct from_bytes<size_t>
            {
                size_t operator()(const uint8_t*& data, size_t& size)
                {
                    ...
                    {
                        throw decode_error(
                          fmt::format("Trying to decode number: {}  is too many bytes for uint64_t", size));
                    }
            ...
            struct from_bytes
            {
                std::enable_if_t<std::is_integral_v<T> && std::is_unsigned_v<T>, T> operator()(
            ...
            struct from_bytes<std::array<uint8_t, N>>
            {
                std::array<uint8_t, N> operator()(const uint8_t*& data, size_t& size)
                {
                    ...
                    {
                        throw decode_error(
                          fmt::format("Trying to decode {} byte array, but given {} bytes to decode", N, size));
                    }
            ...
            template <typename... Ts>
            std::tuple<Ts...> decode_tuple(const uint8_t*& data, size_t& size, std::tuple<Ts...>);

            // Specialisation for decoding empty tuples
            template <>
            inline std::tuple<> decode_tuple(const uint8_t*& data, size_t& size, std::tuple<>)
            {
            ...
            inline std::tuple<T, Ts...> decode_tuple(const uint8_t*& data, size_t& size, std::tuple<T, Ts...>)
            {
                const auto first = decode_impl<T>(data, size);

                return std::tuple_cat(first, decode_tuple<Ts...>(data, size, std::tuple<Ts...>{}));
            }
            ...
            std::tuple<Ts...> decode_impl(const uint8_t*& data, size_t& size)
            {
                ...
                if constexpr (sizeof...(Ts) == 0)
                {
                    ...
                    {
                        throw decode_error(
                          fmt::format("Expected empty list, but data contains {} remaining bytes", contained_length));
                    }
        ...
        std::tuple<Ts...> decode(const uint8_t*& data, size_t& size)
        {
            ...
            if (size != 0)
            {
                throw decode_error(fmt::format("Expected to decode entire input, but {} bytes remain", size));
            }
