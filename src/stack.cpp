...
namespace eevm
{
    ...
    uint64_t Stack::pop64()
    {
        ...
        if (val > numeric_limits<uint64_t>::max())
            throw Exception(ET::outOfBounds, "Value on stack (" + to_hex_string(val) + ") is larger than 2^64");
    ...
    void Stack::push(const uint256_t& val)
    {
        if (size() == MAX_SIZE)
            throw Exception(
              ET::outOfBounds, "Stack mem exceeded (" + to_string(size()) + " == " + to_string(MAX_SIZE) + ")");
    ...
    {
        if (i >= size())
            throw Exception(ET::outOfBounds, "Swap out of range (" + to_string(i) + " >= " + to_string(size()) + ")");
    ...
    {
        if (a >= size())
            throw Exception(ET::outOfBounds, "Dup out of range (" + to_string(a) + " >= " + to_string(size()) + ")");
